...
public class Ex5_Format {
	...
	public static void main(String[] args) {
		// <format 클래스>
		//DecimalFormat
		//1) 금액이 저장된 변수를 ,를 이용하여 표현하는 예제
		int num = 1230000;
		DecimalFormat format = new DecimalFormat("#,###");
		String numStr = format.format(num);
		System.out.println(numStr + "원");
		
		//2) 월에서 1-9월이면 0을 붙이는 예제
		//int month = 01; -> 8진수라는 의미
		int month = 1;
		...
		String msFormat = "이아디 : {0}, 이름 : {1}, 번호 : {2}";
		//MessageFormat.format(문자열 포맷, 객체들)을 이용하여 문자열 포맷에 값들을 대입한 후 문자열 생성
		String user2 = MessageFormat.format(msFormat, id, name, number);
		System.out.println(user2);
		//배열에서 사용하면 더 편하게 사용할 수 있다 
